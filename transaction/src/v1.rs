...
//!
//! This format is incompatible with serde/bincode, so Transaction only supports
//! raw byte serialization via [`Transaction::serialize`] and [`Transaction::from_bytes`].

use {
    crate::versioned::VersionedTransaction,
    solana_message::{
        v1::{Message, MessageError},
        VersionedMessage,
    },
...
impl From<MessageError> for TransactionError {
...
impl Transaction {
    ...
    #[cfg(feature = "verify")]
    pub fn verify_with_results(&self) -> Result<Vec<bool>, TransactionError> {
        ...
        if self.message.account_keys.len() < required {
            return Err(TransactionError::NotEnoughAccountKeys);
        }
    ...
    /// Calculate the size of this transaction when serialized to wire format.
    /// This includes the message bytes plus all signatures.
...
/// Convert a Transaction into a VersionedTransaction.
...
/// Try to convert a VersionedTransaction into a Transaction.
///
...
mod tests {
    ...
    fn try_sign_rejects_too_many_signers() {
        ...
        // Provide 3 signers
        let result = Transaction::try_sign(message, &[&keypair0, &keypair1, &keypair2]);
    ...
    fn try_sign_rejects_not_enough_signers() {
        ...
        let result = Transaction::try_sign(message, &[&keypair0]);
    ...
    fn try_sign_rejects_wrong_keypairs() {
        ...
        let result = Transaction::try_sign(message, &[&wrong_keypair]);
    ...
    fn try_sign_signs_correctly() {
        ...
        let tx = Transaction::try_sign(message, &[&keypair]).unwrap();
    ...
    fn try_sign_with_multiple_signers() {
        ...
        let tx = Transaction::try_sign(message, &[&keypair0, &keypair1]).unwrap();
    ...
    fn try_sign_reorders_keypairs_correctly() {
        ...
        let tx = Transaction::try_sign(message, &[&keypair1, &keypair0]).unwrap();
    ...
    fn from_bytes_rejects_invalid_message() {
        ...
        assert!(matches!(result, Err(TransactionError::MessageError(_))));

        // Invalid version byte - fails at message parsing
        let result = Transaction::from_bytes(&[0x00]);
        assert!(matches!(result, Err(TransactionError::MessageError(_))));
    }
    ...
    fn verify_rejects_not_enough_account_keys() {
        ...
        let result = tx.verify_with_results();
        assert_eq!(result, Err(TransactionError::NotEnoughAccountKeys));
    }
    ...
    fn conversion_from_versioned_v1_rejects_signature_count_mismatch() {
        ...
        let result = Transaction::try_from(versioned);
    ...
    fn conversion_from_versioned_v1_succeeds() {
        ...
        let tx = Transaction::try_from(versioned).unwrap();
    ...
    #[test]
    fn serialized_size_includes_all_signatures() {
        // Create message with max signatures (12)
        let mut account_keys: Vec<_> = (0..MAX_SIGNATURES).map(|_| Address::new_unique()).collect();
        account_keys.push(Address::new_unique()); // program

        let message = Message::builder()
            .num_required_signatures(MAX_SIGNATURES)
            .lifetime_specifier(Hash::new_unique())
            .account_keys(account_keys)
            .instruction(CompiledInstruction {
                program_id_index: MAX_SIGNATURES,
                accounts: (0..MAX_SIGNATURES).collect(),
                data: vec![],
            })
            .build()
            .unwrap();
        ...
        // Verify signatures are at the expected position
        let message_size = tx.message.size();
        assert_eq!(bytes.len(), message_size + (MAX_SIGNATURES as usize * 64));
    }

    #[test]
    fn max_size_transaction_exactly_4096_bytes() {
        // Build a transaction that's exactly at the 4096 byte limit
        // Fixed header: 1 + 3 + 4 + 32 + 1 + 1 = 42 bytes
        // With 1 signer: 1 signature = 64 bytes
        // With 2 addresses: 64 bytes
        // Total fixed: 42 + 64 + 64 = 170 bytes
        // Remaining for instruction: 4096 - 170 = 3926 bytes
        // Instruction header: 4 bytes
        // Instruction accounts: 1 byte
        // Instruction data: 3926 - 4 - 1 = 3921 bytes
    ...
    #[test]
    fn transaction_exceeding_4096_bytes_fails_sanitize() {
        // Construct message directly to bypass builder size validation.
        // This tests that sanitize() catches oversized transactions.
        let message = Message {
            header: MessageHeader {
                num_required_signatures: 1,
                num_readonly_signed_accounts: 0,
            ...
            instructions: vec![CompiledInstruction {
                program_id_index: 1,
                accounts: vec![0],
                data: vec![0u8; 3922], // One more byte than max
            }],
        };
    ...
    #[test]
    fn max_signatures_with_max_addresses_fits() {
        // Transaction with 12 signers and 64 addresses
        let account_keys: Vec<_> = (0..MAX_ADDRESSES).map(|_| Address::new_unique()).collect();
        ...
        let message = Message::builder()
            ...
            .account_keys(account_keys)
            .instruction(CompiledInstruction {
                program_id_index: MAX_SIGNATURES, // First non-signer as program
                accounts: (0..MAX_SIGNATURES).collect(),
                data: vec![],
            })
            .build()
        ...
        // 64 addresses: 2048 bytes
        // 12 signatures: 768 bytes
        // 1 instruction header: 4 bytes
        // 12 account indices: 12 bytes
        // Total: 42 + 2048 + 768 + 4 + 12 = 2874 bytes
    ...
    fn error_display_formats_correctly() {
        assert_eq!(
            TransactionError::MessageError(MessageError::BufferTooSmall).to_string(),
            "message error: buffer too small"
        );
        assert_eq!(
            TransactionError::NotEnoughAccountKeys.to_string(),
            "not enough account keys for required signatures"
        );
        assert_eq!(
            TransactionError::NotEnoughSignatureBytes.to_string(),
            "not enough bytes for signatures"
        );
        assert_eq!(
            TransactionError::Overflow.to_string(),
            "size calculation overflow"
        );
        assert_eq!(
            TransactionError::SignatureCountMismatch {
                expected: 2,
                actual: 1
            }
            .to_string(),
        ...
        assert_eq!(
            TransactionError::TrailingData.to_string(),
            "unexpected trailing data after transaction"
        );
    }
