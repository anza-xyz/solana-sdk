...
/// bytes as [`serde_varint`](crate), so wincode and bincode are wire-compatible.
///
...
/// use solana_serde_varint::wincode::Leb128Int;
///
    ...
    ($type:ty) => {
        unsafe impl<C: ConfigCore> SchemaWrite<C> for Leb128Int<$type> {
            ...
            fn size_of(src: &$type) -> WriteResult<usize> {
                let bits = <$type>::BITS - src.leading_zeros();
...
mod tests {
    ...
    struct Dummy {
        #[serde(with = "crate")]
        #[wincode(with = "crate::wincode::Leb128Int<u16>")]
        a: u16,
        #[serde(with = "crate")]
        #[wincode(with = "crate::wincode::Leb128Int<u32>")]
        b: u32,
        #[serde(with = "crate")]
        #[wincode(with = "crate::wincode::Leb128Int<u64>")]
        c: u64,
    }
    ...
    #[test]
    fn edge_cases() {
        // Values at varint byte boundaries for each field.
        let cases = [
            Dummy { a: 0, b: 0, c: 0 },
            Dummy { a: 1, b: 1, c: 1 },
            Dummy {
    ...
    #[test]
    fn trailing_zeros() {
        // [0x80, 0x00] triggers "Invalid Trailing Zeros" on field `a` (u16).
        let buf = [0x80u8, 0x00];
    ...
    fn last_byte_truncated() {
        // Field `a` = 1 (single byte), then `b` ends with a byte whose high bits
        // get truncated when shifted into u32.
    ...
    #[test]
    fn shift_overflow() {
        // Three bytes all with continuation bit set exhausts u16's 3-byte budget.
    ...
    #[test]
    fn short_buffer() {
        // Single byte with continuation bit set — field `a` is cut off.
        let buf = [0x80u8];
        let r = wincode::deserialize::<Dummy>(&buf);
