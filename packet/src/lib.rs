...
/// Marker for packet types which are cheaply cloneable.
pub trait ZeroCopyPacket {}
...
impl ZeroCopyPacket for Packet {}
...
impl ZeroCopyPacket for PacketMut {}
...
impl PacketArray {
    ...
    where
        T: serde::Serialize,
    {
        let mut packet = PacketArray::default();
        packet.size = bincode::serialized_size(&data)? as usize;
...
impl PacketRead for PacketArray {
    fn data<I>(&self, index: I) -> Option<&<I as SliceIndex<[u8]>>::Output>
    where
        I: SliceIndex<[u8]>,
    {
        if self.meta.discard() {
            ...
            // initialized.
            let data = unsafe { mem::transmute::<_, &[u8]>(&self.buffer[..self.size]) };
