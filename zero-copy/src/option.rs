//! Generic zero-copy `Option`-like wrapper for types that can reserve a
//! designated null value without adding a tag byte.
//!
...
/// A zero-copy wrapper that can be used as an `Option<T>` without
